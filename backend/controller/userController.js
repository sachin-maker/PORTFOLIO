...
export const register = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    ...
    if (!cloudinaryResponseForAvatar || cloudinaryResponseForAvatar.error) {
      return next(
        new ErrorHandler(
          cloudinaryResponseForAvatar.error?.message || "Failed to upload avatar",
          500
        )
      );
    }
    ...
    console.log("Cloudinary Resume Response:", cloudinaryResponseForResume);

    if (!cloudinaryResponseForResume || cloudinaryResponseForResume.error) {
      return next(
        new ErrorHandler(
          cloudinaryResponseForResume.error?.message || "Failed to upload resume",
          500
        )
      );
    }
...
export const getUserForPortfolio = catchAsyncErrors(async (req, res, next) => {
  const id = "67abf59ff2df99471424651f";
